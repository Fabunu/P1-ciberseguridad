...
RUTA_BASE_GRYPE = Path(__file__).resolve().parents[2]
RUTA_REPOS_POR_DEFECTO = RUTA_BASE_GRYPE / "data" / "repos"
RUTA_RESULTADOS_POR_DEFECTO = RUTA_BASE_GRYPE / "data" / "raw" / "grype"
...
class GrypeAnalyzer:
    ...
    def run(self):
        ...
        for indice, repo_path in enumerate(repositorios, start=1):
            ...
            # =========================================================
            # NUEVO: skip si ya existen outputs
            # =========================================================
            ruta_raw = (
                self.output_path /
                f"{ruta_repo.name}{SUFIJO_GRYPE_RAW}"
            )

            ruta_normalizado = (
                self.output_path /
                f"{ruta_repo.name}{SUFIJO_GRYPE}"
            )

            if ruta_raw.exists() and ruta_normalizado.exists():
                LOGGER.info(
                    "[%s/%s] Grype ya existe para %s. Se omite.",
                    indice,
                    len(repositorios),
                    ruta_repo.name,
                )
            ...
            try:
                ...
                self._eliminar_archivos_parciales(
                    ruta_repo.name
                )

                LOGGER.error(
                    f"Error scanning {repo_path}: {error}"
                )
