...
RUTA_BASE_SBOMS = Path(__file__).resolve().parents[2]
RUTA_REPOS_POR_DEFECTO = RUTA_BASE_SBOMS / "data" / "repos"
RUTA_RESULTADOS_POR_DEFECTO = RUTA_BASE_SBOMS / "data" / "raw" / "syft"
...
class SBOMGenerator:
    ...
    def run(self):
        ...
        for indice, repo_path in enumerate(repositorios, start=1):
            ...
            # =========================================================
            # NUEVO: skip si ya existe
            # =========================================================
            if ruta_salida.exists():
                LOGGER.info(
                    "[%s/%s] SBOM ya existe para %s. Se omite.",
                    indice,
                    len(repositorios),
                    ruta_repo.name,
                )
