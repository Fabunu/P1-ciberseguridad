...
RUTA_RESULTADOS_POR_DEFECTO = RUTA_BASE_CODEQL / "data" / "raw" / "codeql"
...
class CodeQLAnalyzer:
    ...
    def run(self):
        ...
        for indice, repo_path in enumerate(
            repositorios,
            start=1,
        ):
            ruta_repo = (
            ...
            # =====================================================
            # NUEVO:
            # skip si ya existe el output de CodeQL
            # =====================================================
            ruta_salida = (
                self.output_path /
            ...
            if ruta_salida.exists():
                LOGGER.info(
                    "[%s/%s] CodeQL ya existe para %s. Se omite.",
                    indice,
                    len(repositorios),
                    ruta_repo.name,
                )
            ...
            # =====================================================
            try:
                sarif_data = self.run_codeql(
                    repo_path
                )

                analysis = self.parse_sarif(
                    sarif_data
                )
